...
export class NewCommand extends Command {
    ...
    async execute(args: string[]): Promise<IHistoryCommand> {
        if (args.length < 2) {
            return new HistoryCommand(this.getCommandName(), args, "missing parameters (e.g.: new goblin adrien)", "error-component");
        } else {
            ...
            if ("block-component" === block.templateName) { //todo this is bad code
                try {
                    ...
                    console.log(e);
                    return new HistoryCommand(this.getCommandName(), args, monsterName + " already exists in the encounter.", "error-component");
                }
            } else {
                return new HistoryCommand(this.getCommandName(), args, monsterType + " does not exists.", "error-component");
            }
        }
    }
}