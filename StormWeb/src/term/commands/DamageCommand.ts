...
export class DamageCommand extends Command {
    ...
    async execute(args: string[]): Promise<IHistoryCommand> {
        if (args.length < 2) {
            return new HistoryCommand(this.getCommandName(), args, "missing parameters (e.g.: damage adrien 2)", "error-component");
        } else {
            const monsterName = args[0];
            const monsterDamage = -parseInt(args[1]);
            if (isNaN(monsterDamage)) {
                return new HistoryCommand(this.getCommandName(), args, 'The request should be like "damage adrien 2".', "error-component")
            }
